...
function SettingsPage() {
  ...
  const save = (event: React.FormEvent) => {
    ...
    if (budget <= 0 || balance < 0) {
      toast.error("Check your amounts");
      return;
    }
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Card className="rounded-2xl border-border/60 p-5 shadow-[var(--shadow-soft)] sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-lg font-semibold tracking-tight">Preferences</h2>
              <p className="text-sm text-muted-foreground">
                These values power your local dashboard.
              </p>
            </div>
          </div>
          <form onSubmit={save} className="mt-6 space-y-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="settings-balance">Opening balance (₹)</Label>
                <Input
                  id="settings-balance"
                  type="number"
                  min="0"
        ...
        <Card className="rounded-2xl border-border/60 p-5 shadow-[var(--shadow-soft)] sm:p-6">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent className="rounded-2xl">
              <AlertDialogHeader>
                <AlertDialogTitle className="font-display">Reset all demo data?</AlertDialogTitle>
                <AlertDialogDescription>
                  This restores the original Finance Tracker sample data and removes local changes.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
